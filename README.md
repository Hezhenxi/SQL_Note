...
<a name="hhkj-1710687364421"></a>[2.   授予权限](#diuh-1698935003460)
...
<a name="xdds-1710687364488"></a>[不支持外键支持表锁，](#tbyo-1705136742374)
...
<a name="xggk-1710687364555"></a>[1.  创建create](#a8zv-1698936024125)
...
<a name="30vk-1710687364621"></a>[-表级锁](#nkxl-1705577226591)
...
<a name="3hk2-1710687364684"></a>[1.  Buffer Pool(减少磁盘IO）](#cbz5-1698936905672)
...
<a name="coei-1710687364749"></a>[3.DB_ROw_ID隐藏主键](#thdk-1699301971563)
...
<a name="pala-1710687364816"></a>[. 介绍](#ebw6-1704227729277)
...
-但是其他事务同样update可以在同一间隙上采用间隙锁，间隙读/间隙写
...
1．意向共享锁（IS)︰与表锁共享锁( read）s兼容，与表锁排它锁(write) x互斥。

2．意向排他锁（IX)∶与表锁共享锁(read)s及排它锁(write)x都互斥。意向锁之间不会互斥。即IS和IX兼容

没锁具体的数据，提醒有人有意向加锁

object\_schema    object\_name     index\_name    lock\_type    lock\_mode    lock\_data

heimaSQL    stu    TABLEIX

heimaSQL    stuT    ABLEIS
...
`      `情况1.并发写：不管什么隔离级别(强调的是读)下，写操作都需要防止脏写(parallel write)覆盖其它事务的写数据，导致数据丢失lost update,

`  `-------原子写则保证每个写入都会被记录保存，不被覆盖丢失，物理覆盖，版本链上无记录---------

`      `情况2，写依赖读  即读取-修改-写入序列

` `（从业务代码层面理解：写依赖读）因为业务写大多会依赖数据可见性，即需要读现数据，需串行执行保证一致性.

`   `------ 不希望读取后数据被其它事务修改。导致更新的增量逻辑上的被覆盖，以保证基础原子写，版本链上有记录。
...
INSERT ...排他锁    自动加锁   --添加IX + X,REC\_NOT\_GAP, 有其他写入时才会查到X,REC\_NOT\_GAP
...
select(正常)                                                                                  IS + S,REC\_NOT\_GAP   -隐式 lock in share mode

lock in share mode          IS + S,REC\_NOT\_GAP     IS + S,REC\_NOT\_GAP    IS + S,REC\_NOT\_GAP   IS + S,REC\_NOT\_GAP

update/for update           IX + X,REC\_NOT\_GAP     IX + X,REC\_NOT\_GAP    IX + X,REC\_NOT\_GAP   IX + X,REC\_NOT\_GAP

lock in share mode(null)         IS                     IS                  IS+S,GAP                IS+S,GAP         

update/for update (null)         IX                     IX                  IX+X,GAP                IX+X,GAP

IX IS 各个事务各持一把锁

间隙锁唯一目的是防止其他事务插入间隙（insert）。间隙锁可以共存，一个事务采用的间隙锁不会阻止另一个事务在同一间隙上采用间隙锁(select 显式锁锁/ update )。

读不建议使用lock in share mode 和 for update的原因：(还得看具体场景，并不绝对)

不走索引会锁全表，较记录来说，粒度过大 -不走索引锁数据等价于查全量数据加锁select \* from stu share/for update;

且查空数据时--

读已提交：无间隙锁，不能解决幻读/写偏差问题

可重复读：添加间隙锁，死锁的风险。-mysql会自动检测释放


SELECT @@TRANSACTION\_ISOLATION;

SET SESSION TRANSACTI ON ISOLATION LEVEL  READ UNCOMMITTED --可单独在会话中设置改事务的隔离级别;
...
--IS-意向共享锁,此时for update会阻塞，但也会尝试往data\_locks里面插入锁记录(X,REC\_NOT\_GAP inser\_intention + IX)，

结果：超时会自动中止，释放阻塞的行锁，IX意向锁插入成功(事务中止/结束则释放)。
...
<a name="icwc-1710687085045"></a>**是对空数据仅阻止插入，不妨碍其他事务获得共享间隙锁或排它间隙锁：**

<a name="eemq-1710687094582"></a>**即 间隙锁 事务A 拥有S,GAP  仅阻塞insert,  不会阻碍 事务B select for update / update  X,GAP**

<a name="1rze-1710507690759"></a>**如果双方还都持有间隙读锁或者间隙写锁(同个间隙，key虽然不同但是间隙锁范围一样)，就互相阻塞写操作，死锁**

<a name="fxrw-1710507714031"></a>●间隙锁/临键锁-演示

默认情况下，InnoDB在REPEATABLE READ事务隔离级别运行，InnoDB使用next-key锁（临建锁）进行搜索和索引扫描，以防止幻读。

id=16~100 为空  

事务A执行select \* from stu where id=55 lock in share mode

null数据/读已提交下：仅加了意向锁

object\_schema    object\_name     index\_name    lock\_type    lock\_mode    lock\_data

heimaSQL    stu                  TABLE    IS

事务B  insert into stu values (55,'Ruby',55) 可正常插入

可重复读下/串行化下： 加意向锁和临键锁 =当前读
...
heimaSQL    stu            PRIMARYRECORD        S,GAP        100

heimaSQL    stu                   TABLEIX                    -事务B

heimaSQL    stu            PRIMARYRECORD    X,GAP        100         -事务B  select for update /update

heimaSQL    stu            PRIMARYRECORDX,GAP,INSERT\_INTENTION100         -事务B  inssert

事务B 

`    `select for update/ update stu set name='Peter' where id=55 > Affected rows: 0 

`   `--正常来说，是s,gap 和 x,gap 是互斥的，像阻塞insert那样， haha,be fun. 

`   `--合理解释是对空数据仅阻止插入，不妨碍其他事务获得共享间隙锁或排它间隙锁

`   `insert into stu values (55,'Ruby',55) 阻塞
...
SELECT \* from stu where id =5 lock in SHARE mode;

SELECT \* from stu where id =5 for UPDATE

SELECT \* from stu where id =55 lock in SHARE mode;

SELECT \* from stu where id =55 for update

update stu set name='Peter' where id=55

insert into stu values (55,'Ruby',7)

<a name="lvdd-1698786993879"></a>ROLLBACK

<a name="2cip-1698785368743"></a>**3.  临键 锁**

<a name="llfe-1708969143884"></a>**s  行锁+间隙锁；  s,reco\_not\_gap 行锁； s, gap 间隙锁**

<a name="zgvp-1708969096165"></a>**2.索引上的等值查询=(普通索引)，向右遍历时最后一个值不满足查询需求时，next-key lock退化为间隙锁。-防止插入同样的值导致幻读**
...
如:select ..lock in share mode(共享锁)- 注意：lock in share mode仅限隔离级别为可重复读/串行化下为当前读。
