...
Read uncommitted        √(会出现)  √           √           无                无

Read committed                   √            √             写锁          每次读取生成readview版本视图

Repeatable Read(默认)                         √            读写锁         复用第一次select生成的readview-可重复读

Serializable                                              读写锁             无,读会阻塞写   
...
1．共享锁(S）∶允许一个事务去读一行，阻止其他事务获得相同数据集的排它锁。简单来说共享锁和共享锁兼容，与排他锁互斥。
...
s (共享锁)    兼容        冲突                -与其他事务，当前事务时可以同时获取读写锁的。
...
`  `--IS-意向共享锁,此时for update会阻塞，但也会往data\_locks里面插入锁记录，超时会自动关事务，释放阻塞的行锁，意向锁还在。
...
B-udpate stu set name='Peter' where id=3 ; --阻塞，虽然id不同(不同行)，但是name查询不走索引，升级表锁，所有数据被加锁
...
A先SELECT \* FROM your\_table WHERE column BETWEEN 10 AND 20 锁[10,20] 或 如下记录仅有id  1 3 8 11 19 25 update where id=5(无该行记录)，锁3~8间隙（3,8）
